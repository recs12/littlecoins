...
def card_debit():
    ...
    transactions = df_index_by_dates[['Acc.', 'Description', 'Cheque','Expenses', 'Incomes']].sort_index()
    transactions['card'] = 'debit'
    return transactions
...
def card_credit():
    ...
    frais = df_total[df_total['Description'].str.contains(REGX).values]  
    frais['card'] = 'credit' 
    print(frais.columns)
    transactions = df_total[~df_total['Description'].str.contains(REGX).values]
    transactions['card'] = 'credit' 
